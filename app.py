...
DEFAULT_MODEL_NAME = "GPT OSS 120B (Recommended)"
...
with st.sidebar:
    ...
    with st.container(border=True):
        ...
        <div class="sidebar-header-pill" style="margin-top: 0; margin-bottom: 0.8rem;">
            <span class="icon">⚡</span>
            <span class="text">AI Model Engine</span>
        </div>
        """, unsafe_allow_html=True)
        selected_model_name = st.selectbox(
            ...
            help="Select the AI model for resume evaluation."
        )
            ...
            if errs:
                ...
                if len(uploaded_files) == 1:
                    ...
                    with st.status(f"Analyzing resume: {f.name}...", expanded=True) as status:
                        ...
                        result = analyze_resume(resume_text, job_description, api_key, selected_model_id)
                    ...
                    with st.status(f"Analyzing {len(uploaded_files)} resumes...", expanded=True) as status:
                        ...
                        if success_count > 0:
                            ...
                            st.session_state.analysis_data = None
                            st.session_state.resume_text = None
                            st.session_state.sel_model = selected_model_name
                            st.rerun()
                        else:
                            status.update(label="Analysis failed", state="error")
