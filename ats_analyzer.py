...
def analyze_resume(
    ...
    """
    # Automatically remap decommissioned Groq models
    ...
    if model in decommissioned_models:
        model = decommissioned_models[model]
...
Analyze the resume against the job description. Respond ONLY with valid JSON matching the specified format. Keep explanations concise, structured, and focused (1-2 sentences per field) to ensure a complete, valid JSON output without truncation. No markdown, no explanation outside JSON.
"""
    ...
    if api_key.strip().startswith("AIzaSy"):
        ...
        try:
            try:
                chat_completion = _call_groq(model)
            except Exception as e:
                err_str = str(e).lower()
                # If model not found or decommissioned, automatically try fallback models
                if "not found" in err_str or "does not exist" in err_str or "decommissioned" in err_str:
                    fallback_order = ["openai/gpt-oss-120b", "openai/gpt-oss-20b", "qwen/qwen3.8-27b"]
                    chat_completion = None
                    for fb_model in fallback_order:
                        if fb_model != model:
                            try:
                                chat_completion = _call_groq(fb_model)
                                break
                            except Exception:
                                continue
